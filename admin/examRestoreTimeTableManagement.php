	...
	<main>
		...
		<section>
			<article class="archiveData">
					<?php 
						$getData = $pdo -> prepare("SELECT * from etimetables  WHERE archiveID = 1");
						$getData->execute();

						foreach ($getData as $storedValue) 
						{
							echo '<li>' .$storedValue['eday']." ".$storedValue['edate'] .'<a href ="examRestoreTimeTableManagement.php?archId='.$storedValue['etid'].'"> Restore </a> </li>';
						}
					?>
			</article>
		</section>
