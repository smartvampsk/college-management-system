	...
	<main>
		...
		<section>
			<article class="archiveData">
					<?php 
						$getData = $pdo -> prepare("SELECT * from timetables WHERE archiveID = 1");
						$getData->execute();

						foreach ($getData as $storedValue) 
						{
							echo '<li>' .$storedValue['tday']." ".$storedValue['tdate'] .'<a href ="restoreTimeTableManagement.php?archId='.$storedValue['tid'].'"> Restore </a> </li>';
						}
					?>
			</article>
		</section>
